...
class Database:
    ...
    def _populate_sample_data(self):
        ...
        sample_signatures = [
            ("44d88612fea8a8f36de82e1278abb02f", "Test Malware", "Sample malicious file", "high"),
            ("e1112134b6dcc8bed54e0cf67ec1043c", "Suspicious Code", "Known malicious pattern", "medium"),
            ("a2s3f4g5h6j7k8l9p0o1i2u3y4t5r6e", "Ransomware.Generic", "Generic ransomware signature", "critical")
        ]
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            if cursor.fetchone()[0] == 0:
                conn.executemany(
                    "INSERT INTO signatures (hash, name, description, severity) VALUES (?, ?, ?, ?)",
                    sample_signatures
                )
    ...
    def add_signature(self, hash: str, name: str, description: str, severity: str):
        """Add a new signature to the database."""
        with sqlite3.connect(self.db_path) as conn:
            conn.execute(
                "INSERT INTO signatures (hash, name, description, severity) VALUES (?, ?, ?, ?)",
                (hash, name, description, severity)
            )
